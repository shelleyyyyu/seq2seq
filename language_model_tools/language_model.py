...
class Language_Model():
    def __init__(self, args, infer=False):
        ...
        lm_saver = tf.train.Saver(tf.global_variables())
        self.saver = lm_saver


    def sample(self, sess, words, vocab, num, prime='first all', sampling_type=1, pick=1, width=4, quiet=False):
    ...
    def getProb(self, sess, prime, targetWord, vocab):
        state = sess.run(self.cell.zero_state(1, tf.float32))
        ...
        p = probs[0]
        target_word_index = -1
        if dict(vocab).has_key(targetWord) == True:
            target_word_index = dict(vocab).get(targetWord)

        return p[target_word_index]
